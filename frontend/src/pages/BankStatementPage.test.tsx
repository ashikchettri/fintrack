...
const EMPTY_HOUSEHOLD = {
  currency: null, month: null, availableMonths: [], totalShared: 0, memberCount: 0, fairShare: 0,
...
describe('BankStatementPage', () => {
  it('shows the upload call-to-action when there are no transactions', async () => {
    ...
    renderWithProviders(<BankStatementPage />, ['/bank']);
  ...
  it('renders charts, merchants, recent activity and household sharing', async () => {
  ...
  it('scopes to a month via the period selector', async () => {
    mockedApi.dashboard.mockResolvedValue(POPULATED);
    renderWithProviders(<BankStatementPage />, ['/bank']);
  ...
  it('surfaces an error when the statement cannot load', async () => {
    mockedApi.dashboard.mockRejectedValue(new ApiError(500, { title: 'Server error', status: 500 }));
    renderWithProviders(<BankStatementPage />, ['/bank']);

    expect(await screen.findByText(/could not load your statement/i)).toBeInTheDocument();
  });
